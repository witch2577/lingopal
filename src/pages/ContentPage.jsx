  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="mb-3 sm:mb-4">
        <h1 className="text-lg sm:text-xl font-bold text-theme-primary">发现</h1>
        <p className="text-xs sm:text-sm text-theme-muted">探索更多学习内容和社交功能</p>
      </div>

      <ContentUpdater />

      {/* Top tab bar */}
      <div className="flex bg-theme-elevated rounded-2xl p-1 mb-3 sm:mb-4 overflow-x-auto hide-scrollbar">
        {tabs.map(tab => (
          <button
            key={tab.key}
...
const SocialHub = () => {
  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      <div className="flex bg-theme-elevated rounded-2xl p-1">
        {tabs.map(tab => (
          <button
            key={tab.key}
            onClick={() => setActiveTab(tab.key)}
