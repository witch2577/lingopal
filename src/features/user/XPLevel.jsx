    ...
    <div className="flex flex-col gap-4">
      ...
      <Card className="bg-gradient-to-br from-brand-500 to-violet-600 text-white" padding="p-5">
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 rounded-2xl bg-theme-card/20 backdrop-blur-sm flex items-center justify-center">
          ...
          <div className="flex-1">
            ...
            <div className="h-3 bg-theme-card/20 rounded-full overflow-hidden">
              <motion.div
      ...
      {/* XP Sources */}
      <Card padding="p-4">
        <h3 className="font-semibold text-theme-primary text-sm mb-3">经验获取方式</h3>
        <div className="space-y-2">
          {Object.entries(XP_SOURCES).map(([key, src]) => (
            <div key={key} className="flex items-center justify-between p-2.5 bg-theme-elevated rounded-xl">
              <span className="text-sm text-theme-secondary">{src.label}</span>
              <div className="flex items-center gap-1">
                <span className="text-sm font-medium text-emerald-600">
                  +{src.base}{src.perDay ? '/天' : ''}
                </span>
                <span className="text-xs text-theme-muted">XP</span>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Weekly XP bar chart */}
      <Card padding="p-4">
        <h3 className="font-semibold text-theme-primary text-sm mb-3">近7天经验获取</h3>
          ...
          {weekData.map(([date, amount], i) => {
            ...
            return (
              <div key={date} className="flex-1 flex flex-col items-center gap-1">
                ...
                <span className="text-[10px] text-theme-muted">{date.slice(5).replace('-', '/')}</span>
              </div>
            );
          })}
        </div>
      ...
      {/* Level roadmap */}
      <Card padding="p-4">
        <h3 className="font-semibold text-theme-primary text-sm mb-3">等级进阶路线</h3>
            ...
            return (
              <div
                ...
                className={`flex items-center gap-3 p-3 rounded-xl ${
                  isCurrent ? 'bg-brand-50 border border-brand-200' : reached ? 'bg-theme-elevated' : 'bg-transparent opacity-50'
                }`}
              >
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold ${
                  reached ? 'bg-brand-500 text-white' : 'bg-theme-elevated text-theme-muted'
                }`}>
                  {lv.level}
                </div>
                <div className="flex-1 min-w-0">
                  <div className={`text-sm font-medium ${isCurrent ? 'text-brand-700' : 'text-theme-secondary'}`}>
                    {lv.title}
                  </div>
                  <div className="text-[10px] text-theme-muted">{lv.xpNeeded} XP</div>
                </div>
                {lv.unlocks.length > 0 && (
