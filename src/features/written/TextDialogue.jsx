    ...
    return (
      <div className="flex flex-col gap-4">
        <div className="text-center mb-2">
          <h2 className="text-lg font-bold text-theme-primary">选择对话场景</h2>
          <p className="text-sm text-theme-muted">在情境中练习文字对话</p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-3">
          {scenarios.map(([key, scene]) => (
            <motion.button
              ...
              className="flex items-start gap-3 p-4 bg-theme-card rounded-2xl border border-theme-light text-left hover:border-brand-300 hover:shadow-md transition-all btn-press"
            >
              ...
              <div className="flex-1 min-w-0">
                <h3 className="font-semibold text-theme-primary">{scene.name}</h3>
                <p className="text-xs text-theme-muted mt-0.5">{scene.description}</p>
              </div>
              <Icon name="chevron" size={18} className="text-theme-muted flex-shrink-0 mt-2" />
            </motion.button>
          ))}
        </div>
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="flex-1">
          <h2 className="text-base font-bold text-theme-primary">{scene.name}</h2>
          <p className="text-xs text-theme-muted">{scene.description}</p>
        </div>
        {isComplete && (
              ...
              <div
                className={`max-w-[80%] px-4 py-2.5 rounded-2xl text-sm ${
                  msg.role === 'user'
                    ...
                    : 'bg-theme-elevated text-theme-secondary rounded-bl-md'
                }`}
              >
                {msg.text}
              </div>
        ...
        <div className="flex flex-col gap-2">
            ...
            <div className="grid grid-cols-1 gap-2">
              {currentUserTurn.options.map((opt, i) => (
                <motion.button
                  ...
                  className="flex items-center gap-3 p-3 bg-theme-card border-2 border-theme-light rounded-xl text-left text-sm hover:border-brand-400 hover:text-brand-700 transition-all disabled:opacity-50"
                >
                  <span className="w-6 h-6 rounded-full bg-theme-elevated text-theme-muted text-xs flex items-center justify-center font-medium flex-shrink-0">
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-4 py-2.5 rounded-xl border-2 border-theme-light focus:border-brand-400 focus:outline-none text-sm transition-colors"
              disabled={feedback !== null}
            />
            <Button
              variant="primary"
      ...
      {isComplete && (
        ...
        >
          <div className="text-center">
            <h3 className="text-lg font-bold text-theme-primary">对话完成！</h3>
