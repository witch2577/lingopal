...
const SkeletonLine = ({ width = '100%', height = '16px', className = '' }) => (
  <div
    className={`bg-theme-elevated rounded animate-pulse ${className}`}
    style={{ width, height }}
  />
);
...
const SkeletonCircle = ({ size = '40px', className = '' }) => (
  <div
    className={`bg-theme-elevated rounded-full animate-pulse ${className}`}
    style={{ width: size, height: size }}
  />
);

const SkeletonCard = ({ lines = 3, hasHeader = true, className = '' }) => (
  <div className={`bg-theme-card rounded-2xl border border-theme-light p-4 space-y-3 ${className}`}>
    {hasHeader && (
...
const SkeletonList = ({ count = 4, className = '' }) => (
  <div className={`space-y-3 ${className}`}>
    {Array.from({ length: count }).map((_, i) => (
      <div key={i} className="flex items-center gap-3 bg-theme-card rounded-xl p-3 border border-theme-light">
...
const SkeletonGrid = ({ rows = 2, cols = 2, className = '' }) => (
  <div className={`grid grid-cols-${cols} gap-3 ${className}`}>
    {Array.from({ length: rows * cols }).map((_, i) => (
      <div key={i} className="bg-theme-card rounded-2xl border border-theme-light p-4 space-y-3">
...
const SkeletonHeatmap = ({ className = '' }) => (
  <div className={`bg-theme-card rounded-2xl border border-theme-light p-4 ${className}`}>
    <SkeletonLine width="40%" height="16px" className="mb-3" />
    <div className="grid grid-cols-7 gap-1">
      {Array.from({ length: 28 }).map((_, i) => (
        <div key={i} className="aspect-square rounded bg-theme-elevated animate-pulse" style={{ animationDelay: `${i * 0.02}s` }} />
      ))}
    </div>
  </div>
);
...
const SkeletonChart = ({ className = '' }) => (
  <div className={`bg-theme-card rounded-2xl border border-theme-light p-4 ${className}`}>
    ...
    <div className="h-40 bg-theme-elevated rounded-xl animate-pulse flex items-end justify-around px-4 pb-4 gap-2">
      {Array.from({ length: 8 }).map((_, i) => (
        <div
          key={i}
          className="w-full bg-theme-elevated rounded-t-lg animate-pulse"
        ...
        <div className="flex items-end justify-center gap-3 py-4">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="flex flex-col items-center gap-2">
              <SkeletonCircle size="48px" />
              <div className="w-16 bg-theme-elevated rounded-t-xl animate-pulse" style={{ height: `${60 + i * 20}px` }} />
            </div>
          ))}
        </div>
